...
<table border=2px >
         ...
         <marquee><b>USER TABLE INFORMATION</b></marquee><br><br>
	...
	<th>Update</th>
         ...
         List<UserModel> get = userService.getUserById(userId);
        for(UserModel re:get)
        {
    		...
    		out.println("</td>");
    		out.println("<td>");%>
    		<a href="UpdateUser.jsp?x=<%= re.getUserId() %>&a=<%= re.getName() %>&b=<%=re.getDOB()%>&c=<%=re.getGender()%>&d=<%=re.getPanNo()%>&e=<%=re.getSalary()%>&f=<%=re.getMobNo()%>&g=<%=re.getNationality()%>">Update</a>
